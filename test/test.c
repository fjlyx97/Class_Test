...
void homepage()//主页
{
	...
	while(1)
	{
		...
		while(1)
		{
			...
			case '3':serch();break;
			case '4':printf("排序");break;//sort();break;
			case '5':delet();break;
			case '6':change();break;
			case '0':ending();break;
...
void serch()
{
	...
	while(1)
	{
		...
		case '3':serch_author();
		case '4':serch_dirive();
		case '5':serch_indistinct();
		case '0':homepage();break;
		default:printf("系统出错！");Sleep(1000);serch();break;
		}
	}
}
void serch_fail()//查找失败
...
{
	char dlh[10];
	struct book *p1,*p2;   //p1用于查找数据，p2储存上一节点
	char t;
	int flag=0;
	p1=NULL;
	p2=NULL;
	...
	toxy(30,3+n);
	scanf("%d",&dlh);
	for(p1=head;p1!=NULL;p1=p1->next)//按顺序查找
	{
		p2=p1;
		if(strcmp(p1->num,dlh)==0)//找到对应数据
		{
			flag=1;
			if(p1==head) //改变指针域所指向位置
				head=p1->next;
			else
				p2->next=p1->next;
			n--;
...
void change()
{
	...
	while(1)
	{
		...
		if(t=='s')
		{
			scanf("%d%s%s%s%s%d.%d.%d%f",p1->num,p1->name,p1->author,&p1->dirive,p1->publish_house,&p1->printdate.year,&p1->printdate.month,&p1->printdate.day,&p1->ptice);
			p1->next=p2->next;
			p2->next=p1;
		}
		if(t=='e')
